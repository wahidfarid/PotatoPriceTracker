...
async function main() {
  ...
  for (const c of cardsToCheck) {
    ...
    if (card) {
      ...
      for (const v of card.variants) {
        if (c.collectorNumber && v.collectorNumber !== c.collectorNumber)
          continue;
        ...
        console.log(
          `- #${v.collectorNumber} ${v.language} Foil:${v.isFoil} ScryfallID:${v.scryfallId} -> Prices: ${prices.length}`,
        );
        prices.forEach((p) =>
          console.log(`   * ${p.shop.name}: ¥${p.priceYen}`),
        );
      }
    } else {
  ...
  for (const q of queries) {
    ...
    try {
      ...
      const eclItems = docs.filter(
        (d: any) =>
          d.product_name.includes("[ECL]") ||
          d.product_name_en?.includes("[ECL]"),
      );
