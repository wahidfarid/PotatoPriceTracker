...
async function main() {
    ...
    await page.goto(`https://www.hareruyamtg.com/ja/products/search?product=${encodeURIComponent(query)}`);
    ...
    try {
        ...
        const options = await page.$$eval('select[name="cardset"] option', els => els.map(el => ({
            id: el.value,
            text: el.textContent
        })));

        const match = options.find(o => o.text?.includes('Lorwyn') || o.text?.includes('ローウィン'));

        console.log('Matches:', options.filter(o => o.text?.toLowerCase().includes('lorwyn') || o.text?.includes('クリプス')));
    } catch (e) {
