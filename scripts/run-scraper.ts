...
function showUsage(): never {
    ...
    console.error('  tsx scripts/run-scraper.ts hareruya     # run hareruya only');
    console.error('  tsx scripts/run-scraper.ts buy          # run all *_set scrapers');
    console.error('  tsx scripts/run-scraper.ts cardrush sell # run cardrush kaitori only');
    process.exit(1);
}
...
for (const arg of args) {
    if (isShop(arg)) {
        ...
        if (options.priceType) {
            console.error(`Error: Multiple price types specified: ${options.priceType}, ${arg}`);
            showUsage();
        }
