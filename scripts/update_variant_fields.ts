...
async function updateVariantFields() {
  ...
  console.log(
    `Updating ${variants.length} variants with frame/promo information...`,
  );
  ...
  for (const variant of variants) {
    ...
    try {
      const res = await fetch(
        `https://api.scryfall.com/cards/${variant.scryfallId}`,
      );
      if (!res.ok) {
      ...
      if (variant.isFoil) {
        finish =
          scryfallFinishes.includes("etched") &&
          !scryfallFinishes.includes("foil")
            ? "etchedfoil"
            : "foil";
      }
