...
async function verifyKaitoriData() {
  ...
  const totalWithBuyPrice = latestPrices.filter(
    (p) => p.buyPriceYen !== null,
  ).length;
  const totalWithUrl = latestPrices.filter(
    (p) => p.buyPriceYen !== null && p.sellSourceUrl !== null,
  ).length;
  const urlCoverage =
    totalWithBuyPrice > 0
      ? ((totalWithUrl / totalWithBuyPrice) * 100).toFixed(2)
      : "0.00";
  ...
  const sortedSets = Array.from(setStats.entries()).sort(
  ...
  const missingHighValue = await prisma.$queryRaw<
    Array<{
      name: string;
      setCode: string;
  ...
  if (missingHighValue.length === 0) {
    ...
    for (const card of missingHighValue) {
      ...
      console.log(
        `  ${card.name.padEnd(30)} | ${card.setCode} | ${card.collectorNumber.padEnd(4)} | ${card.language} | ${foil.padEnd(4)} | ¥${card.buyPriceYen}`,
      );
    }
  }
